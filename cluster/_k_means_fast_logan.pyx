...
cpdef DOUBLE _assign_labels_array(np.ndarray[floating, ndim=2] X,
    ...
    if flag=="acc":
        print("Using acc gap as the bias metric")
    elif flag == "auc":
        print("using auc gap as the bias metric")
    else:
        print("please check your bias metric")
    ...
    for sample_idx in range(n_samples):
        ...
        for center_idx in range(n_clusters):
            ...
            # adding predictions of each clusters here by jyzhao
            if labels[sample_idx] != center_idx:
                y_clusters = np.concatenate((y[labels == center_idx], y[sample_idx].reshape(1, -1)), axis = 0)
            else:
                y_clusters = y[labels == center_idx]
            cluster_m = y_clusters[y_clusters[:, -1] == 0]
            cluster_f = y_clusters[y_clusters[:, -1] == 1]
            if cluster_m.shape[0] == 0:
                acc_m = 0
            else:
                if flag=="acc":
                    acc_m = accuracy_score(y_true = cluster_m[:,:pidx], y_pred = cluster_m[:, pidx:-1])
                elif flag=="auc":

                    acc_m = roc_auc_score(y_true=cluster_m[:,:pidx], y_score = cluster_m[:, pidx:-1], average = "micro")
            if cluster_f.shape[0] == 0:
                acc_f = 0
            else:
                if flag=="acc":
                    acc_f = accuracy_score(y_true = cluster_f[:, :pidx], y_pred = cluster_f[:,  pidx:-1])
                elif flag=="auc":
                    acc_f = roc_auc_score(y_true = cluster_f[:, :pidx], y_score = cluster_f[:,  pidx:-1], average = "micro") 
            # print(f"dist:{dist}, acc_m:{(acc_m - acc_f) ** 2}")

            odist = dist * sample_weight[sample_idx]
            dist += (acc_m - acc_f) ** 2 * eta
